...
class PolycrystalUserObjectBaseCopy;
struct GrainDistanceCopy;

class GrainTrackerCopy : public FeatureFloodCountCopy, public GrainTrackerInterfaceCopy
{
public:
  static InputParameters validParams();

  GrainTrackerCopy(const InputParameters & parameters);
  virtual ~GrainTrackerCopy();
  ...
  // GrainTrackerInterfaceCopy methods
...
protected:
   ...
   * tracking grains. This method is meant to be used with the PolycrystalUserObjectBaseCopy class
   ...
   */
  void prepopulateState(const FeatureFloodCountCopy & ffc_object);
   ...
   * FeatureData object found by the FeatureFloodCountCopy object.
   */
  void assignGrains();
  ...
  const PolycrystalUserObjectBaseCopy * const _poly_ic_uo;
...
struct GrainDistanceCopy
{
  GrainDistanceCopy(Real distance, std::size_t var_index);

  GrainDistanceCopy(Real distance,
                std::size_t var_index,
                std::size_t grain_index,
                unsigned int grain_id);

  // Copy constructors
  GrainDistanceCopy(const GrainDistanceCopy & f) = default;
  GrainDistanceCopy & operator=(const GrainDistanceCopy & f) = default;

  // Move constructors
  GrainDistanceCopy(GrainDistanceCopy && f) = default;
  GrainDistanceCopy & operator=(GrainDistanceCopy && f) = default;

  bool operator<(const GrainDistanceCopy & rhs) const;
...
template <>
void dataStore(std::ostream & stream, GrainTrackerCopy::PartialFeatureData & feature, void * context);
template <>
void dataLoad(std::istream & stream, GrainTrackerCopy::PartialFeatureData & feature, void * context);