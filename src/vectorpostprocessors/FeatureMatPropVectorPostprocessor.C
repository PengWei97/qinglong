...
registerMooseObject("PhaseFieldApp", FeatureMatPropVectorPostprocessor);

InputParameters
FeatureMatPropVectorPostprocessor::validParams()
{
...
FeatureMatPropVectorPostprocessor::FeatureMatPropVectorPostprocessor(
...
void
FeatureMatPropVectorPostprocessor::initialize()
{
}

void
FeatureMatPropVectorPostprocessor::execute()
{
...
void
FeatureMatPropVectorPostprocessor::finalize()
{
  // Do the parallel sum
...
Real
FeatureMatPropVectorPostprocessor::getFeatureVolume(unsigned int feature_id) const
{
...
void
FeatureMatPropVectorPostprocessor::accumulateVolumes(
    const Elem * elem,
...
Real
FeatureMatPropVectorPostprocessor::computeIntegral(std::size_t var_index) const
{
  Real sum = 0;
...
void
FeatureMatPropVectorPostprocessor::accumulateBoundaryFaces(
    const Elem * elem,
    const std::vector<unsigned int> & var_to_features,
    std::size_t libmesh_dbg_var(num_features),
    unsigned int side)
...
Real
FeatureMatPropVectorPostprocessor::computeFaceIntegral(std::size_t var_index) const
{
  Real sum = 0;
