...
#include "ComputeMultipleCrystalPlasticityStressCopy.h"
...
registerMooseObject("TensorMechanicsApp", ComputeMultipleCrystalPlasticityStressCopy);

InputParameters
ComputeMultipleCrystalPlasticityStressCopy::validParams()
{
  ...
  params.addClassDescription(
      ...
      "that are inherited from CrystalPlasticityStressUpdateBaseCopy");
...
ComputeMultipleCrystalPlasticityStressCopy::ComputeMultipleCrystalPlasticityStressCopy(
...
void
ComputeMultipleCrystalPlasticityStressCopy::initQpStatefulProperties()
{
...
void
ComputeMultipleCrystalPlasticityStressCopy::initialSetup()
{
  ...
  for (unsigned int i = 0; i < _num_models; ++i)
  {
    CrystalPlasticityStressUpdateBaseCopy * model =
        dynamic_cast<CrystalPlasticityStressUpdateBaseCopy *>(&getMaterialByName(model_names[i]));
    ...
    if (model)
    {
      ...
      mooseError("Model " + model_names[i] +
                 " is not compatible with ComputeMultipleCrystalPlasticityStressCopy");
  }
...
void
ComputeMultipleCrystalPlasticityStressCopy::computeQpStress()
{
  for (unsigned int i = 0; i < _num_models; ++i)
    _models[i]->setQp(_qp);
...
void
ComputeMultipleCrystalPlasticityStressCopy::updateStress(RankTwoTensor & cauchy_stress,
                                                     RankFourTensor & jacobian_mult)
{
  ...
  do
  {
    ...
    if (substep_iter > _max_substep_iter && _convergence_failed)
      mooseException("ComputeMultipleCrystalPlasticityStressCopy: Constitutive failure");
...
void
ComputeMultipleCrystalPlasticityStressCopy::preSolveQp()
{
  for (unsigned int i = 0; i < _num_models; ++i)
...
void
ComputeMultipleCrystalPlasticityStressCopy::solveQp()
{
  for (unsigned int i = 0; i < _num_models; ++i)
  {
...
void
ComputeMultipleCrystalPlasticityStressCopy::postSolveQp(RankTwoTensor & cauchy_stress,
                                                    RankFourTensor & jacobian_mult)
{
...
void
ComputeMultipleCrystalPlasticityStressCopy::solveStateVariables()
{
  ...
  do
  {
    ...
    {
      if (_print_convergence_message)
        mooseWarning("ComputeMultipleCrystalPlasticityStressCopy: State variables (or the system "
  ...
  if (iteration == _maxiterg)
  {
    if (_print_convergence_message)
      mooseWarning(
          "ComputeMultipleCrystalPlasticityStressCopy: Hardness Integration error. Reached the "
...
void
ComputeMultipleCrystalPlasticityStressCopy::solveStress()
{
  ...
  {
    if (_print_convergence_message)
      mooseWarning("ComputeMultipleCrystalPlasticityStressCopy: the slip increment exceeds tolerance "
                   "at element ",
                   _current_elem->id(),
                   " and Gauss point ",
                   _qp);
  ...
  while (rnorm > _rtol * rnorm0 && rnorm > _abs_tol && iteration < _maxiter)
  {
    ...
    {
      if (_print_convergence_message)
        mooseWarning("ComputeMultipleCrystalPlasticityStressCopy: Failed with line search");
  ...
  {
    if (_print_convergence_message)
      mooseWarning("ComputeMultipleCrystalPlasticityStressCopy: Stress Integration error rmax = ",
                   rnorm,
...
void
ComputeMultipleCrystalPlasticityStressCopy::calculateResidualAndJacobian()
{
  calculateResidual();
  if (_convergence_failed)
    return;
...
void
ComputeMultipleCrystalPlasticityStressCopy::calculateResidual()
{
...
void
ComputeMultipleCrystalPlasticityStressCopy::calculateJacobian()
{
...
void
ComputeMultipleCrystalPlasticityStressCopy::calcTangentModuli(RankFourTensor & jacobian_mult)
{
...
void
ComputeMultipleCrystalPlasticityStressCopy::elastoPlasticTangentModuli(RankFourTensor & jacobian_mult)
{
...
void
ComputeMultipleCrystalPlasticityStressCopy::elasticTangentModuli(RankFourTensor & jacobian_mult)
{
...
bool
ComputeMultipleCrystalPlasticityStressCopy::lineSearchUpdate(const Real & rnorm_prev,
...
void
ComputeMultipleCrystalPlasticityStressCopy::calculateEigenstrainDeformationGrad()
{
  _inverse_eigenstrain_deformation_grad.zero();
  _inverse_eigenstrain_deformation_grad.addIa(1.0);
