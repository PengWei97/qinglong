...
#include "ComputeMultCPStressCpl.h"
...
registerMooseObject("TensorMechanicsApp", ComputeMultCPStressCpl);

InputParameters
ComputeMultCPStressCpl::validParams()
{
...
ComputeMultCPStressCpl::ComputeMultCPStressCpl(
...
void
ComputeMultCPStressCpl::initQpStatefulProperties()
{
...
void
ComputeMultCPStressCpl::initialSetup()
{
  ...
  for (unsigned int i = 0; i < _num_models; ++i)
  {
    ...
    if (model)
    {
      ...
      mooseError("Model " + model_names[i] +
                 " is not compatible with ComputeMultCPStressCpl");
  }
...
void
ComputeMultCPStressCpl::computeQpStress()
{
  for (unsigned int i = 0; i < _num_models; ++i)
    _models[i]->setQp(_qp);
...
void
ComputeMultCPStressCpl::updateStress(RankTwoTensor & cauchy_stress,
                                                     RankFourTensor & jacobian_mult)
{
  ...
  do
  {
    ...
    if (substep_iter > _max_substep_iter && _convergence_failed)
      mooseException("ComputeMultCPStressCpl: Constitutive failure");
...
void
ComputeMultCPStressCpl::preSolveQp()
{
  for (unsigned int i = 0; i < _num_models; ++i)
...
void
ComputeMultCPStressCpl::solveQp()
{
  for (unsigned int i = 0; i < _num_models; ++i)
  {
...
void
ComputeMultCPStressCpl::postSolveQp(RankTwoTensor & cauchy_stress,
                                                    RankFourTensor & jacobian_mult)
{
...
void
ComputeMultCPStressCpl::solveStateVariables()
{
  ...
  do
  {
    ...
    {
      if (_print_convergence_message)
        mooseWarning("ComputeMultCPStressCpl: State variables (or the system "
  ...
  if (iteration == _maxiterg)
  {
    if (_print_convergence_message)
      mooseWarning(
          "ComputeMultCPStressCpl: Hardness Integration error. Reached the "
...
void
ComputeMultCPStressCpl::solveStress()
{
  ...
  {
    if (_print_convergence_message)
      mooseWarning("ComputeMultCPStressCpl: the slip increment exceeds tolerance "
                   "at element ",
                   _current_elem->id(),
                   " and Gauss point ",
                   _qp);
  ...
  while (rnorm > _rtol * rnorm0 && rnorm > _abs_tol && iteration < _maxiter)
  {
    ...
    {
      if (_print_convergence_message)
        mooseWarning("ComputeMultCPStressCpl: Failed with line search");
  ...
  {
    if (_print_convergence_message)
      mooseWarning("ComputeMultCPStressCpl: Stress Integration error rmax = ",
                   rnorm,
...
void
ComputeMultCPStressCpl::calculateResidualAndJacobian()
{
  calculateResidual();
  if (_convergence_failed)
    return;
...
void
ComputeMultCPStressCpl::calculateResidual()
{
...
void
ComputeMultCPStressCpl::calculateJacobian()
{
...
void
ComputeMultCPStressCpl::calcTangentModuli(RankFourTensor & jacobian_mult)
{
...
void
ComputeMultCPStressCpl::elastoPlasticTangentModuli(RankFourTensor & jacobian_mult)
{
...
void
ComputeMultCPStressCpl::elasticTangentModuli(RankFourTensor & jacobian_mult)
{
...
bool
ComputeMultCPStressCpl::lineSearchUpdate(const Real & rnorm_prev,
...
void
ComputeMultCPStressCpl::calculateEigenstrainDeformationGrad()
{
  _inverse_eigenstrain_deformation_grad.zero();
  _inverse_eigenstrain_deformation_grad.addIa(1.0);
