...
InputParameters
CrystalPlasticityStressUpdateBaseCopy::validParams()
{
...
CrystalPlasticityStressUpdateBaseCopy::CrystalPlasticityStressUpdateBaseCopy(
...
void
CrystalPlasticityStressUpdateBaseCopy::initQpStatefulProperties()
{
...
void
CrystalPlasticityStressUpdateBaseCopy::getSlipSystems()
{
  ...
  if (orthonormal_error)
    mooseError("CrystalPlasticityStressUpdateBaseCopy Error: The slip system file contains a slip "
               "direction and plane normal pair that are not orthonormal in the Cartesian "
               "coordinate system.");
}

void
CrystalPlasticityStressUpdateBaseCopy::transformHexagonalMillerBravaisSlipSystems(
    const MooseUtils::DelimitedFileReader & reader)
{
  ...
  if (_unit_cell_dimension[0] != _unit_cell_dimension[1] ||
    ...
    mooseError("CrystalPlasticityStressUpdateBaseCopy Error: The specified unit cell dimensions are "
               "not consistent with expectations for "
               "HCP crystal hexagonal lattices.");
  else if (reader.getData(0).size() != miller_bravais_indices * 2)
    mooseError("CrystalPlasticityStressUpdateBaseCopy Error: The number of entries in the first row of "
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    for (const auto j : index_range(reader.getData(i)))
    {
      ...
      if (basal_pl_sum > _zero_tol)
        mooseError(
            "CrystalPlasticityStressUpdateBaseCopy Error: The specified HCP basal plane Miller-Bravais "
      ...
      if (basal_dir_sum > _zero_tol)
        mooseError("CrystalPlasticityStressUpdateBaseCopy Error: The specified HCP slip direction "
...
void
CrystalPlasticityStressUpdateBaseCopy::sortCrossSlipFamilies()
{
...
unsigned int
CrystalPlasticityStressUpdateBaseCopy::identifyCrossSlipFamily(const unsigned int index)
{
  for (unsigned int i = 0; i < _number_cross_slip_directions; ++i)
    for (unsigned int j = 0; j < _number_cross_slip_planes; ++j)
...
void
CrystalPlasticityStressUpdateBaseCopy::calculateFlowDirection(const RankTwoTensor & crysrot)
{
...
void
CrystalPlasticityStressUpdateBaseCopy::calculateSchmidTensor(
...
void
CrystalPlasticityStressUpdateBaseCopy::calculateShearStress(
...
void
CrystalPlasticityStressUpdateBaseCopy::calculateTotalPlasticDeformationGradientDerivative(
...
void
CrystalPlasticityStressUpdateBaseCopy::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityStressUpdateBaseCopy::setQp(const unsigned int & qp)
{
  _qp = qp;
}

void
CrystalPlasticityStressUpdateBaseCopy::setSubstepDt(const Real & substep_dt)
{
  _substep_dt = substep_dt;
}

bool
CrystalPlasticityStressUpdateBaseCopy::isConstitutiveStateVariableConverged(
