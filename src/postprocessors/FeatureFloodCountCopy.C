...
#include "FeatureFloodCountCopy.h"
...
template <>
void
dataStore(std::ostream & stream, FeatureFloodCountCopy::FeatureData & feature, void * context)
{
  /**
...
template <>
void
dataLoad(std::istream & stream, FeatureFloodCountCopy::FeatureData & feature, void * context)
{
  /**
...
FeatureFloodCountCopy::validParams()
{
  ...
  /**
   * The FeatureFloodCountCopy and derived objects should not to operate on the displaced mesh. These
  ...
  // The FeatureFloodCountCopy object does not require that any state (restartable information) is
...
FeatureFloodCountCopy::FeatureFloodCountCopy(const InputParameters & parameters)
...
void
FeatureFloodCountCopy::initialSetup()
{
...
void
FeatureFloodCountCopy::initialize()
{
...
void
FeatureFloodCountCopy::clearDataStructures()
{
}

void
FeatureFloodCountCopy::meshChanged()
{
...
FeatureFloodCountCopy::execute()
{
  ...
  {
    mooseInfo("Using EXPERIMENTAL boundary restricted FeatureFloodCountCopy object!\n");
...
processor_id_type
FeatureFloodCountCopy::numberOfDistributedMergeHelpers() const
{
  return _app.n_processors() >= _maps_size ? _maps_size : 1;
}

void
FeatureFloodCountCopy::communicateAndMerge()
{
  ...
  if (n_merging_procs > 1)
  {
    ...
    {
      /**
       * The FeatureFloodCountCopy and derived objects rely on having the original data structures
...
void
FeatureFloodCountCopy::sortAndLabel()
{
...
void
FeatureFloodCountCopy::buildLocalToGlobalIndices(std::vector<std::size_t> & local_to_global_all,
                                             std::vector<int> & counts) const
{
  ...
  local_to_global_all.resize(globalsize, FeatureFloodCountCopy::invalid_size_t);
  for (const auto & feature : _feature_sets)
  {
...
void
FeatureFloodCountCopy::buildFeatureIdToLocalIndices(unsigned int max_id)
{
...
void
FeatureFloodCountCopy::finalize()
{
...
FeatureFloodCountCopy::getVarToFeatureVector(dof_id_type elem_id) const
{
...
void
FeatureFloodCountCopy::scatterAndUpdateRanks()
{
  ...
  if (!_is_primary)
  {
    ...
    for (auto & list_ref : _partial_feature_sets)
    {
      for (auto & feature : list_ref)
      {
        ...
        auto global_index = FeatureFloodCountCopy::invalid_size_t;
        ...
        if (global_index != FeatureFloodCountCopy::invalid_size_t)
        {
  ...
  else
  {
    for (auto global_index : local_to_global_all)
      if (global_index != FeatureFloodCountCopy::invalid_size_t && global_index > largest_global_index)
        largest_global_index = global_index;
  }
...
Real
FeatureFloodCountCopy::getValue() const
{
  return static_cast<Real>(_feature_count);
}

std::size_t
FeatureFloodCountCopy::getNumberActiveFeatures() const
{
  // Note: This value is parallel consistent, see FeatureFloodCountCopy::communicateAndMerge()
  return _feature_count;
}

std::size_t
FeatureFloodCountCopy::getTotalFeatureCount() const
{
  /**
   * Since the FeatureFloodCountCopy object doesn't maintain any information about
...
unsigned int
FeatureFloodCountCopy::getFeatureVar(unsigned int feature_id) const
{
...
bool
FeatureFloodCountCopy::doesFeatureIntersectSpecifiedBoundary(unsigned int feature_id) const
{
  // Some processors don't contain the largest feature id, in that case we just return invalid_id
  if (feature_id >= _feature_id_to_local_index.size())
    return false;
...
bool
FeatureFloodCountCopy::isFeaturePercolated(unsigned int feature_id) const
{
  // Some processors don't contain the largest feature id, in that case we just return invalid_id
  if (feature_id >= _feature_id_to_local_index.size())
    return false;
...
Point
FeatureFloodCountCopy::featureCentroid(unsigned int feature_id) const
{
  if (feature_id >= _feature_id_to_local_index.size())
    return invalid_id;
...
Real
FeatureFloodCountCopy::getEntityValue(dof_id_type entity_id,
...
void
FeatureFloodCountCopy::prepareDataForTransfer()
{
  ...
  for (auto & list_ref : _partial_feature_sets)
  {
    for (auto & feature : list_ref)
    {
       ...
       */
      FeatureFloodCountCopy::sort(feature._ghosted_ids);
      FeatureFloodCountCopy::sort(feature._local_ids);
      FeatureFloodCountCopy::sort(feature._halo_ids);
      FeatureFloodCountCopy::sort(feature._disjoint_halo_ids);
      FeatureFloodCountCopy::sort(feature._periodic_nodes);
...
void
FeatureFloodCountCopy::serialize(std::string & serialized_buffer, unsigned int var_num)
{
...
void
FeatureFloodCountCopy::deserialize(std::vector<std::string> & serialized_buffers, unsigned int var_num)
{
...
void
FeatureFloodCountCopy::mergeSets()
{
...
void
FeatureFloodCountCopy::consolidateMergedFeatures(std::vector<std::list<FeatureData>> * saved_data)
{
  ...
  /**
   * IMPORTANT: FeatureFloodCountCopy::_feature_count is set on rank 0 at this point but
   * we can't broadcast it here because this routine is not collective.
   */
}

bool
FeatureFloodCountCopy::areFeaturesMergeable(const FeatureData & f1, const FeatureData & f2) const
{
  return f1.mergeable(f2);
}

void
FeatureFloodCountCopy::updateFieldInfo()
{
  for (const auto i : index_range(_feature_sets))
  {
...
bool
FeatureFloodCountCopy::flood(const DofObject * dof_object, std::size_t current_index)
...
Real FeatureFloodCountCopy::getThreshold(std::size_t /*current_index*/) const
{
  return _step_threshold;
}

Real FeatureFloodCountCopy::getConnectingThreshold(std::size_t /*current_index*/) const
{
...
bool
FeatureFloodCountCopy::isNewFeatureOrConnectedRegion(const DofObject * dof_object,
...
void
FeatureFloodCountCopy::expandPointHalos()
{
...
void
FeatureFloodCountCopy::expandEdgeHalos(unsigned int num_layers_to_expand)
{
...
void
FeatureFloodCountCopy::visitElementalNeighbors(const Elem * elem,
                                           FeatureData * feature,
...
void
FeatureFloodCountCopy::visitNodalNeighbors(const Node * node,
                                       FeatureData * feature,
                                       bool expand_halos_only)
{
...
template <typename T>
void
FeatureFloodCountCopy::visitNeighborsHelper(const T * curr_entity,
  ...
  for (const auto neighbor : neighbor_entities)
  {
    if (neighbor && (!_is_boundary_restricted || isBoundaryEntity(neighbor)))
    {
      if (expand_halos_only)
      {
        ...
        else if (!FeatureFloodCountCopy::contains(feature->_local_ids, entity_id))
          feature->_halo_ids.insert(feature->_halo_ids.end(), entity_id);
      }
      else
      {
...
void
FeatureFloodCountCopy::updateBoundaryIntersections(FeatureData & feature) const
{
  if (_is_elemental)
  {
    for (auto entity : feature._local_ids)
...
void
FeatureFloodCountCopy::appendPeriodicNeighborNodes(FeatureData & feature) const
{
  if (_is_elemental)
  {
    for (auto entity : feature._local_ids)
...
template <typename T>
bool
FeatureFloodCountCopy::isBoundaryEntity(const T * entity) const
{
...
void
FeatureFloodCountCopy::FeatureData::updateBBoxExtremes(MeshBase & mesh)
{
  ...
  FeatureFloodCountCopy::reserve(all_primary_region_ids, _local_ids.size() + _halo_ids.size());
  ...
  FeatureFloodCountCopy::reserve(set_union, _halo_ids.size() + _disjoint_halo_ids.size());
...
void
FeatureFloodCountCopy::FeatureData::updateBBoxExtremes(BoundingBox & bbox, const BoundingBox & rhs_bbox)
{
...
bool
FeatureFloodCountCopy::FeatureData::boundingBoxesIntersect(const FeatureData & rhs) const
{
...
bool
FeatureFloodCountCopy::FeatureData::halosIntersect(const FeatureData & rhs) const
{
  return MooseUtils::setsIntersect(_halo_ids, rhs._halo_ids);
}

bool
FeatureFloodCountCopy::FeatureData::periodicBoundariesIntersect(const FeatureData & rhs) const
{
  return MooseUtils::setsIntersect(_periodic_nodes, rhs._periodic_nodes);
}

bool
FeatureFloodCountCopy::FeatureData::ghostedIntersect(const FeatureData & rhs) const
{
  return MooseUtils::setsIntersect(_ghosted_ids, rhs._ghosted_ids);
}

bool
FeatureFloodCountCopy::FeatureData::mergeable(const FeatureData & rhs) const
{
...
bool
FeatureFloodCountCopy::FeatureData::canConsolidate(const FeatureData & rhs) const
{
...
void
FeatureFloodCountCopy::FeatureData::merge(FeatureData && rhs)
{
  ...
  FeatureFloodCountCopy::reserve(set_union, _local_ids.size() + rhs._local_ids.size());
  std::set_union(_local_ids.begin(),
                 _local_ids.end(),
  ...
  set_union.clear();
  FeatureFloodCountCopy::reserve(set_union, _periodic_nodes.size() + rhs._periodic_nodes.size());
  ...
  set_union.clear();
  FeatureFloodCountCopy::reserve(set_union, _ghosted_ids.size() + rhs._ghosted_ids.size());
  ...
  set_union.clear();
  FeatureFloodCountCopy::reserve(set_union, _disjoint_halo_ids.size() + rhs._disjoint_halo_ids.size());
  ...
  set_union.clear();
  FeatureFloodCountCopy::reserve(set_union, _halo_ids.size() + rhs._halo_ids.size());
  std::set_union(_halo_ids.begin(),
                 _halo_ids.end(),
                 rhs._halo_ids.begin(),
                 rhs._halo_ids.end(),
...
void
FeatureFloodCountCopy::FeatureData::consolidate(FeatureData && rhs)
{
  mooseAssert(_var_index == rhs._var_index, "Mismatched variable index in merge");
  mooseAssert(_id == rhs._id, "Mismatched auxiliary id in merge");

  FeatureData::container_type set_union;
  FeatureFloodCountCopy::reserve(_local_ids, _local_ids.size() + rhs._local_ids.size());
  std::set_union(_local_ids.begin(),
                 _local_ids.end(),
                 rhs._local_ids.begin(),
                 rhs._local_ids.end(),
...
void
FeatureFloodCountCopy::FeatureData::clear()
{
  _local_ids.clear();
...
void
FeatureFloodCountCopy::FeatureData::mergeBBoxes(std::vector<BoundingBox> & bboxes,
...
std::ostream &
operator<<(std::ostream & out, const FeatureFloodCountCopy::FeatureData & feature)
{
  static const bool debug = true;

  out << "Grain ID: ";
  if (feature._id != FeatureFloodCountCopy::invalid_id)
    out << feature._id;
  else
    out << "invalid";
  ...
  out << "\nStatus: ";
  if (feature._status == FeatureFloodCountCopy::Status::CLEAR)
    out << "CLEAR";
  if (static_cast<bool>(feature._status & FeatureFloodCountCopy::Status::MARKED))
    out << " MARKED";
  if (static_cast<bool>(feature._status & FeatureFloodCountCopy::Status::DIRTY))
    out << " DIRTY";
  if (static_cast<bool>(feature._status & FeatureFloodCountCopy::Status::INACTIVE))
    out << " INACTIVE";
...
// Constants
const std::size_t FeatureFloodCountCopy::invalid_size_t = std::numeric_limits<std::size_t>::max();
