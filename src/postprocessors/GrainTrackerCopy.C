...
#include "GrainTrackerCopy.h"

// MOOSE includes
#include "PolycrystalUserObjectBaseCopy.h"
...
template <>
void
dataStore(std::ostream & stream, GrainTrackerCopy::PartialFeatureData & feature, void * context)
{
...
template <>
void
dataLoad(std::istream & stream, GrainTrackerCopy::PartialFeatureData & feature, void * context)
{
...
registerMooseObject("qinglongApp", GrainTrackerCopy);

InputParameters
GrainTrackerCopy::validParams()
{
  InputParameters params = FeatureFloodCountCopy::validParams();
  params += GrainTrackerInterfaceCopy::validParams();

  // FeatureFloodCountCopy adds a relationship manager, but we need to extend that for GrainTrackerCopy
  ...
  // The GrainTrackerCopy requires non-volatile storage for tracking grains across invocations.
...
GrainTrackerCopy::GrainTrackerCopy(const InputParameters & parameters)
  : FeatureFloodCountCopy(parameters),
    GrainTrackerInterfaceCopy(),
    ...
    _poly_ic_uo(parameters.isParamValid("polycrystal_ic_uo")
                    ? &getUserObject<PolycrystalUserObjectBaseCopy>("polycrystal_ic_uo")
                    : nullptr),
...
GrainTrackerCopy::~GrainTrackerCopy() {}

Real
GrainTrackerCopy::getEntityValue(dof_id_type entity_id,
                             ...
                             std::size_t var_index) const
{
  ...
  return FeatureFloodCountCopy::getEntityValue(entity_id, field_type, var_index);
}

const std::vector<unsigned int> &
GrainTrackerCopy::getVarToFeatureVector(dof_id_type elem_id) const
{
  return FeatureFloodCountCopy::getVarToFeatureVector(elem_id);
}

unsigned int
GrainTrackerCopy::getFeatureVar(unsigned int feature_id) const
{
  return FeatureFloodCountCopy::getFeatureVar(feature_id);
}

std::size_t
GrainTrackerCopy::getNumberActiveGrains() const
{
...
std::size_t
GrainTrackerCopy::getTotalFeatureCount() const
{
...
Point
GrainTrackerCopy::getGrainCentroid(unsigned int grain_id) const
{
  ...
  if (grain_index != invalid_size_t)
  {
    ...
    // Note: This value is parallel consistent, see GrainTrackerCopy::broadcastAndUpdateGrainData()
...
bool
GrainTrackerCopy::doesFeatureIntersectBoundary(unsigned int feature_id) const
{
...
bool
GrainTrackerCopy::doesFeatureIntersectSpecifiedBoundary(unsigned int feature_id) const
{
  // TODO: This data structure may need to be turned into a Multimap
  mooseAssert(feature_id < _feature_id_to_local_index.size(), "Grain ID out of bounds");
...
bool
GrainTrackerCopy::isFeaturePercolated(unsigned int feature_id) const
{
  // TODO: This data structure may need to be turned into a Multimap
  mooseAssert(feature_id < _feature_id_to_local_index.size(), "Grain ID out of bounds");
...
Real
GrainTrackerCopy::getThreshold(std::size_t var_index) const
{
...
void
GrainTrackerCopy::prepopulateState(const FeatureFloodCountCopy & ffc_object)
{
  ...
  /**
   * The minimum information needed to bootstrap the GrainTrackerCopy is as follows:
   * _feature_sets
   * _feature_count
   */
  if (_is_primary)
...
void
GrainTrackerCopy::finalize()
{
  // Don't track grains if the current simulation step is before the specified tracking step
  if (_t_step < _tracking_step)
    return;

  TIME_SECTION("finalize", 3, "Finalizing GrainTrackerCopy");
  ...
  if (_merge_grains_based_misorientaion && (_t_step > _tracking_step) && (_t_step < 40))
  ...
  if (_verbosity_level > 0)
    _console << "Finished inside of GrainTrackerCopy\n" << std::endl;
}

void
GrainTrackerCopy::mergeGrainsBasedMisorientation()
{
...
void
GrainTrackerCopy::broadcastAndUpdateGrainData()
{
  ...
  if (_is_primary)
  {
    ...
    for (const auto grain_num : index_range(_feature_sets))
    {
      ...
      if (grain._status == Status::CLEAR)
      {
        ...
        if (grain._status == Status::CLEAR)
        {
          auto new_index = 10; // getNextUniqueID();
    ...
    if (_merge_grains_based_misorientaion && (_t_step > 2) && (_t_step < 40))
...
void
GrainTrackerCopy::newGrainCreated(unsigned int new_grain_id)
{
...
GrainTrackerCopy::getNewGrainIDs() const
{
      ...
      for (const auto j : index_range(_feature_sets))
      {
        ...
        if (i < j && grain1._id == grain2._id)
        {
          ...
          if (grain1._var_index != grain2._var_index)
          {
             ...
             * what we want and hope it all works out. Make the GrainTrackerCopy great again!
             */
...
void
GrainTrackerCopy::remapMisorientedGrains()
{
}

void
GrainTrackerCopy::computeMinDistancesFromGrain(FeatureData & grain,
                                           std::vector<std::list<GrainDistanceCopy>> & min_distances)
{
  /**
  ...
  for (const auto i : index_range(_feature_sets))
  {
    ...
    GrainDistanceCopy grain_distance_obj(
...
bool
GrainTrackerCopy::attemptGrainRenumber(FeatureData & grain, unsigned int depth, unsigned int max_depth)
{
  ...
  std::vector<std::list<GrainDistanceCopy>> min_distances(_vars.size());
  ...
  std::sort(min_distances.begin(), min_distances.end(),
            [](const std::list<GrainDistanceCopy> & lhs, const std::list<GrainDistanceCopy> & rhs)
              {
...
void
GrainTrackerCopy::swapSolutionValues(FeatureData & grain,
...
void
GrainTrackerCopy::swapSolutionValuesHelper(Node * curr_node,
...
void
GrainTrackerCopy::updateFieldInfo()
{
...
void
GrainTrackerCopy::communicateHaloMap()
{
  if (_compute_halo_maps)
  {
...
Real
GrainTrackerCopy::centroidRegionDistance(std::vector<BoundingBox> & bboxes1,
...
Real
GrainTrackerCopy::boundingRegionDistance(std::vector<BoundingBox> & bboxes1,
                                     std::vector<BoundingBox> & bboxes2) const
{
  /**
...
unsigned int
GrainTrackerCopy::getNextUniqueID()
{
  /**
 ...
 ************** Helper Structures ****************
 ************************************************/
GrainDistanceCopy::GrainDistanceCopy(Real distance, std::size_t var_index)
  : GrainDistanceCopy(distance,
                  var_index,
...
GrainDistanceCopy::GrainDistanceCopy(Real distance,
...
bool
GrainDistanceCopy::operator<(const GrainDistanceCopy & rhs) const
{
