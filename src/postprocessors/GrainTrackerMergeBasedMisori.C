#include "GrainTrackerMergeBasedMisori.h"

registerMooseObject("yinglongApp", GrainTrackerMergeBasedMisori);

InputParameters
GrainTrackerMergeBasedMisori::validParams()
{
...
GrainTrackerMergeBasedMisori::GrainTrackerMergeBasedMisori(const InputParameters & parameters)
...
void
GrainTrackerMergeBasedMisori::mergeGrainsBasedMisorientation()
{
  ...
  for (const auto grain_num_i : index_range(_feature_sets))
  {
    ...
    for (const auto grain_num_j : index_range(grain_i._adjacent_id))
    {
      ...
      misor_angle = MisoriAngleCalculator::calculateMisorientaion(angles_i, angles_j, _s_misorientation_angle,CrystalType::HCP)._misor;
      ...
      // else if (grain_j._id == 129) // TODO: need to merge based on EBSD data
      //  grain_j._id = 127;
    }
  }
}
...
void 
GrainTrackerMergeBasedMisori::createAdjacentIDVector()
{
  for (const auto grain_num_i : index_range(_feature_sets))
  {
    auto & grain_i = _feature_sets[grain_num_i];
...
void 
GrainTrackerMergeBasedMisori::remapMisorientedGrains()
{
