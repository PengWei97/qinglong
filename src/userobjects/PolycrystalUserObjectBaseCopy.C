...
InputParameters
PolycrystalUserObjectBaseCopy::validParams()
{
  ...
  params.addParam<MooseEnum>("coloring_algorithm",
                             PolycrystalUserObjectBaseCopy::coloringAlgorithms(),
                             PolycrystalUserObjectBaseCopy::coloringAlgorithmDescriptions());

  // FeatureFloodCountCopy adds a relationship manager, but we need to extend that for PolycrystalIC
...
PolycrystalUserObjectBaseCopy::PolycrystalUserObjectBaseCopy(const InputParameters & parameters)
  : FeatureFloodCountCopy(parameters),
    ...
    _num_chunks(FeatureFloodCountCopy::invalid_proc_id)
{
  mooseAssert(_single_map_mode, "Do not turn off single_map_mode with this class");
}

void
PolycrystalUserObjectBaseCopy::initialSetup()
{
  /**
...
void
PolycrystalUserObjectBaseCopy::initialize()
{
...
void
PolycrystalUserObjectBaseCopy::execute()
{
  if (!_colors_assigned)
...
processor_id_type
PolycrystalUserObjectBaseCopy::numberOfDistributedMergeHelpers() const
{
  mooseAssert(_num_chunks != FeatureFloodCountCopy::invalid_proc_id,
...
void
PolycrystalUserObjectBaseCopy::prepareDataForTransfer()
{
   ...
   * support the parallel merge capability that's in the FeatureFloodCountCopy class. We'll need to undo
   * this later, there are a few assumptions built on the sizes of these data structures.
   *
   * See FeatureFloodCountCopy::consolidateMergedFeatures for the "un-sizing" of these structures.
   */
...
void
PolycrystalUserObjectBaseCopy::finalize()
{
  if (_colors_assigned && !_fe_problem.hasInitialAdaptivity())
    return;
  ...
  if (!_colors_assigned)
  {
    ...
    _grain_idx_to_op.resize(_feature_count, PolycrystalUserObjectBaseCopy::INVALID_COLOR);
    if (_is_primary)
    {
...
void
PolycrystalUserObjectBaseCopy::mergeSets()
{
...
void
PolycrystalUserObjectBaseCopy::restoreOriginalDataStructures(std::vector<std::list<FeatureData>> & orig)
{
...
bool
PolycrystalUserObjectBaseCopy::isNewFeatureOrConnectedRegion(const DofObject * dof_object,
...
bool
PolycrystalUserObjectBaseCopy::areFeaturesMergeable(const FeatureData & f1,
...
void
PolycrystalUserObjectBaseCopy::buildGrainAdjacencyMatrix()
{
...
void
PolycrystalUserObjectBaseCopy::assignOpsToGrains()
{
  mooseAssert(_is_primary, "This routine should only be called on the primary rank");
...
bool
PolycrystalUserObjectBaseCopy::colorGraph(unsigned int vertex)
{
  ...
  for (unsigned int color_idx = 0; color_idx < _op_num; ++color_idx)
  {
    ...
    if (isGraphValid(vertex, color))
    {
      ...
      // Backtrack...
      _grain_idx_to_op[vertex] = PolycrystalUserObjectBaseCopy::INVALID_COLOR;
    }
  }
...
bool
PolycrystalUserObjectBaseCopy::isGraphValid(unsigned int vertex, unsigned int color)
{
...
void
PolycrystalUserObjectBaseCopy::printGrainAdjacencyMatrix() const
{
...
const unsigned int PolycrystalUserObjectBaseCopy::INVALID_COLOR =
    std::numeric_limits<unsigned int>::max();
const unsigned int PolycrystalUserObjectBaseCopy::HALO_THICKNESS = 4;